		...
		Mergeable
{
		...
		private void render(
			...
			if ((composite != null) && (composite.getComposite() != null)) {
				...
				graphics.setRenderingHints(
						parentGraphics.getRenderingHints());
			}
			else {
			...
			for (final Pair<PersistableRenderedImage, PersistableComposite> currentStyle : orderedStyles) {
				...
				if ((currentStyle.getValue() == null) || (currentStyle.getValue().getComposite() == null)) {
					...
					graphics.setComposite(
							currentStyle.getValue().getComposite());
				}
				graphics.drawImage(
		...
		public byte[] toBinary() {
			...
			for (final Pair<PersistableRenderedImage, PersistableComposite> style : orderedStyles) {
				...
				if (style != null) {
					...
					final ByteBuffer styleBuf = ByteBuffer.allocate(
							styleCompositeBinary.length + styleImageBinary.length + 4);
					styleBuf.putInt(
							styleCompositeBinary.length);
					if (styleCompositeBinary.length > 0) {
						styleBuf.put(
								styleCompositeBinary);
					}
		...
		public void fromBinary(
			...
			for (int i = 0; i < styleLength; i++) {
				...
				if (styleBinaryLength > 0) {
					...
					buf.get(
							styleBinary);
					final ByteBuffer styleBuf = ByteBuffer.wrap(
							styleBinary);
		...
		public void merge(
			...
			if (merge instanceof CompositeGroupResult) {
				...
				for (int i = 0; i < minStyles; i++) {
					final Pair<PersistableRenderedImage, PersistableComposite> thisStyle = orderedStyles.get(
							i);
					final Pair<PersistableRenderedImage, PersistableComposite> otherStyle = other.orderedStyles.get(
							i);
	...
	public byte[] toBinary() {
		...
		for (final CompositeGroupResult compositeGroup : orderedComposites) {
			...
			compositeBinaries.add(
					compositeGroupBinary);
		}
		final ByteBuffer buf = ByteBuffer.allocate(
				byteSize);
		buf.putInt(
	...
	public void fromBinary(
		...
		for (int i = 0; i < numCompositeGroups; i++) {
			...
			compositeGroup.fromBinary(
					compositeGroupBinary);
			orderedComposites.add(
					compositeGroup);
		}
	}
	...
	public void merge(
		...
		if (merge instanceof DistributedRenderResult) {
			...
			for (int c = 0; c < minComposites; c++) {
				final CompositeGroupResult thisCompositeGroup = orderedComposites.get(
						c);
				final CompositeGroupResult otherCompositeGroup = other.orderedComposites.get(
						c);
				thisCompositeGroup.merge(
						otherCompositeGroup);
				newOrderedComposites.add(
						thisCompositeGroup);
			}
			if (orderedComposites.size() > minComposites) {
				// hopefully this is never the case, but just in case
				newOrderedComposites.addAll(
						orderedComposites.subList(
								minComposites,
								orderedComposites.size()));
			}
			if (other.orderedComposites.size() > minComposites) {
				// hopefully this is never the case, but just in case
				newOrderedComposites.addAll(
						other.orderedComposites.subList(
								minComposites,
								other.orderedComposites.size()));
			}
